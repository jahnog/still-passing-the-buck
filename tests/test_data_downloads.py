...
def test_data_a_1999_committed_workbook_validates():
    """The archived paper-author workbook must stop at 1999."""
    import importlib.util

    script = ROOT / "scripts/validate_data_a_1999.py"
    spec = importlib.util.spec_from_file_location("validate_data_a_1999", script)
    mod = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(mod)

    assert mod.DEST.stat().st_size >= 10_000, "data_a_1999.xlsx suspiciously small"
    mod.validate_data_a_1999(mod.DEST)
...
def test_bcra_exchange_workbook_fetchable():
    ...
    script = ROOT / "scripts/download_bcra_publicaciones_com3500.py"
    spec = importlib.util.spec_from_file_location("download_bcra_com3500", script)
    mod = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(mod)
    ...
    for url in mod.URLS:
        try:
            raw = mod.fetch_bytes(url, timeout=TIMEOUT)
            mod.validate_com3500_xls(raw, source=url)
        except Exception as exc:
            errors.append(f"{url}: {exc}")
            continue
        break
    else:
        pytest.fail("No valid BCRA com3500.xls source responded:\n- " + "\n- ".join(errors))


@pytest.mark.network
def test_indec_ipc_available():
    """INDEC national IPC series used by the official CPI supplement."""
    _requires_network()
