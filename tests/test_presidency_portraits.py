...
def test_notebook_s3_mirror_matches_upload_inventory():
    """Colab fetch list must stay identical to the S3 upload inventory."""
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign):
            ...
            if "S3_MIRROR_FILES" in names:
                mirror = ast.literal_eval(node.value)
    assert mirror is not None, "project_version_badge must define S3_MIRROR_FILES"
    upload = _load_upload_module()
    expected = upload.DATA_FILES + upload.SCRIPT_FILES + upload.ROOT_FILES
    assert tuple(mirror) == expected
    source = "".join(cell["source"])
    assert "_find_project_root" in source
    assert "urlretrieve" in source
    terms_cell = next(c for c in nb["cells"] if c.get("id") == "a1000023")
    assert "base_url=PORTRAIT_BASE_URL" in "".join(terms_cell["source"])
