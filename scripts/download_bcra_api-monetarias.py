...
- 27   Inflación mensual (monthly CPI variation, 1943-03 →) — reproducible CPI source
       for the 1964–2006 inflation variant (IMF IFS routes are not available).
...
remunerated liabilities. Curated year-end anchors in
`generate_fiscal_bcra-quasi-fiscal.py` remain the offline fallback.
...
from scripts.pipeline_config import date_to, target_year

API_BASE = "https://api.bcra.gob.ar/estadisticas/v4.0/monetarias"
# BCRA API filenames extend one year past the CMPI horizon so monthly series stay current.
DATE_TO = date_to(year=target_year() + 1)
PAGE_LIMIT = 3000
...
def fetch_catalog() -> dict[int, dict]:
    ...
    while True:
        ...
        validate_bcra_monetarias_catalog(payload)
...
def main() -> int:
    ...
    for series_id, slug, date_from in SERIES:
        ...
        validate_bcra_monetarias_snapshot(document)
