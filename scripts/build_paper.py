...
Usage:
    ...
    uv run python scripts/build_paper.py --skip-pdf       # extract/resolve only
    uv run python scripts/build_paper.py --output my.pdf

Requirements beyond pyproject.toml: pandoc and a XeLaTeX installation. The
repo ships small compatibility shims under paper/texinputs/ so minimal TeX
installs can still build Pandoc longtables without texlive-latex-extra.
"""
...
# Extra relative width for columns holding strings LaTeX must not break.
ATOMIC_HEADROOM = 1.3
# Smallest share of a table's width that fits "1990–1995" without breaking it.
# Pandoc turns these counts into fractions of the text block, so what a year
# column needs is a share, not a count: digits are wider than the average
# letter this model assumes, and the range cannot be hyphenated.
YEAR_MIN_SHARE = 0.13
# "1990–1995", "2024-2025", "1983→1989" — a span joined by dash, en-dash or arrow.
YEAR_RANGE = re.compile(r"\d{4}\s*[-\u2013\u2014\u2192]\s*\d{2,4}")

# License/archival colophon appended to every built PDF. Kept here, in the
# generator, so the manuscript always carries its license and cannot drift from
# the LICENSE file: CC BY 4.0 (matching the Zenodo record). Mirrors the colophon
...
LICENSE_NOTICE = (
    ...
    "© 2026 Javier Hernan Nogueira. Replication code and data: "
    "<https://github.com/jahnog/still-passing-the-buck> — archived at "
...
TABLES = {
    ...
    "fpi-sensitivity": TableSpec(
        ...
        (
            ("", "Administration"),
            ("Corrected baseline rank", "Corrected baseline"),
            ("Original-study convention rank", "Original-study convention"),
    ...
    "debt-decomposition": TableSpec(
        "debt_decomp_4b",
        (
            ("", "Administration"),
            ("Span", "Span"),
            ("D/Y inherited", "D/Y in"),
            ("D/Y end", "D/Y out"),
            ("D/Y Δ ln%", "D/Y Δ"),
            ("… stock", "stock"),
            ("… real GDP", "real GDP"),
            ("… US$ price", "US$ price"),
            ("… cepo", "cepo"),
            ("… unrecognised", "unrecog."),
        ),
    ),
    "default-window": TableSpec(
        "defaultwindow_9e",
        (
            ("", "Administration"),
            ("CMPI Baseline", "CMPI base"),
            ("CMPI (a) no subst.", "CMPI (a)"),
            ("FPI Baseline", "FPI base"),
            ("FPI (a) no subst.", "FPI (a)"),
            ("Overall Baseline", "Overall base"),
            ("Overall (a) no subst.", "Overall (a)"),
        ),
    ),
    "sensitivity-range": TableSpec(
        "range_9g",
        (
            ("", "Administration"),
            ("Baseline", "Baseline"),
            ("9.5 bare spread", "Bare spread"),
            ("9.7 (a) no default substitution", "No subst."),
            ("Pre-revision interest convention", "Pre-revision"),
            ("9.8 const-price denominators", "Const-price"),
            ("9.8 + 2005 add-backs (high)", "+ add-backs"),
            ("Stacked: pre-revision + denominators", "Stacked"),
        ),
    ),
}
...
def _to_markdown(html: str, spec: TableSpec) -> str:
    ...
    # Headers can't hyphenate, so the longest unbreakable header word sets a
    # floor; so does half the header, which is what a two-line wrap of a
    # "Debt / GDP" style header needs before it splits into three.
    ...
    widths[0] = int(widths[0] * ATOMIC_HEADROOM)
    # A year range is atomic, but LaTeX will happily break it after the dash,
    # dropping the end year onto a second line. Raise those columns to the
    # share that fits the range whole, and only those that fall short, so a
    # table with room to spare keeps the layout it already had.
    for pos, col in enumerate(idx):
        cells = [r[col] for r in data if col < len(r) and r[col]]
        if not (pos and cells and all(YEAR_RANGE.fullmatch(c) for c in cells)):
            continue
        others = sum(widths) - widths[pos]
        widths[pos] = max(widths[pos],
                          ceil(YEAR_MIN_SHARE * others / (1 - YEAR_MIN_SHARE)))
...
def build_pdf(resolved: Path, output: Path) -> None:
    ...
    env = os.environ.copy()
    texinputs = f"{PAPER_DIR / 'texinputs'}//:"
    env["TEXINPUTS"] = texinputs + env.get("TEXINPUTS", "")
    xdg_cache = Path("/tmp/stillpassingthebuck-xdg-cache")
...
def main() -> None:
    ...
    parser.add_argument("--skip-pdf", action="store_true",
                        help="extract notebook artifacts and resolve paper.md without running pandoc")
    ...
    if args.skip_pdf:
        ...
        return
    build_pdf(resolved, args.output)
