...
> The few CSVs still kept under `data/argentina/` are **frozen snapshots** retained only because
> the published companion notebook `Still_Passing_the_Buck.ipynb` fetches them by GitHub raw URL;
> they are *not* read by `Historical_CMPI_Extension.ipynb`. The diverging legacy duplicates
> (`fiscal/fpi-fiscal-1853-2025.csv`, `fiscal/bcra-quasi-fiscal-2001-2025.csv`,
> `historical/historical-cmpi-1853-1963.csv`, `exchange/bcra-dec-dec-1990-1995.csv`) were removed;
> their authoritative versions are the `data/processed/` CSVs regenerated by `scripts/generate_*`.
...
- `Indicators.csv` and `Indicators.csv.gz`: long-form Argentina indicator files consumed by the notebook (now under `data/provided/`).
- `data/argentina/interest/wb-ids-arg.csv`: frozen copy of the annual interest / country-risk series (current: `data/processed/interest/converted_interest_wb-ids-arg_*.csv`).
- `data/argentina/inflation/alt-cpi-2007-2015.csv`: curated alternative-inflation series for the 2007–2015 INDEC intervention (current copy: `data/provided/alt-cpi-2007-2015.csv`; see below).
- `data/argentina/exchange/parallel-cepo.csv`: frozen free-market (CCL/blue) exchange-rate override for the cepo years (current: `data/processed/exchange/converted_exchange_parallel-cepo_*.csv`; see below).
- `data/argentina/exchange/paper-devaluation-1853-1999.csv`: frozen copy of the paper authors' December-quotation devaluation log-diff series for 1853–1999 (current: `data/processed/exchange/converted_exchange_paper-devaluation_*.csv`; see below).
